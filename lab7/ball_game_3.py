...
if not (player_name in data[0]):
    data[0].append(player_name)
    data[1].append(0)

global player_number
player_number = 0
for i in range(len(data[0])):
    if data[0][i] == player_name:
        player_number = i
        print(player_number)
        break

def scoreboard_sort(file_data):
    data = file_data
    global player_number
    for i in range(len(data[0])):
        for j in range(len(data[0]) - 1):
            if int(data[1][j]) < int(data[1][j + 1]):
                print("sorted")
                if j == player_number:
                    player_number += 1
                elif j + 1 == player_number:
                    player_number -= 1
...
def display_scoreboard(file_data, x, y):
    FONT = pygame.freetype.Font("Calibri.ttf", 50)
    data = scoreboard_sort(file_data)
...
finished = False
score = int(data[1][player_number])# Счет игрока
        ...
        if event.type == pygame.QUIT:
            ...
            for i in check_balls_click(balls, event.pos):
                ...
                if balls[i][7] == 1:
                    ...
                    score += SCORE_SQUARE
                balls.pop(i)
            data[1][player_number] = str(score)
